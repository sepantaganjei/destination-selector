...
const DestinationPage = ({params}: any) => {
    ...
    const [travelDestination, setTravelDestination] = useState<TravelDestination | null>(null);
    ...
    return (
        <div>
            ...
            <p><b>Beskrivelse:</b> {travelDestination.description}</p>
            <p><b>Sted:</b> {travelDestination.location}</p>
            <p><b>Tags:</b> {travelDestination.tags.map(tag => <span key={tag}>{tag} </span>)}</p>
        </div>
    );
}
